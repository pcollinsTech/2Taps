export const menuData = {
  starters: [
    {
      title: "1 .Aceitunas Mixtas",
      price: "£4.5",
      description: "Mixed Olives - Ask server for details",
    },
    {
      title: "2. Panes Planos",
      price: "£6",
      description:
        "Grilled flat bread served with beetroot hummus; basil pesto & smoked sundried tomato tapenade",
    },
    {
      title: "3A. Pan Fresco",
      price: "£6",
      description:
        "Selection of bread served with aioli, basil pesto, smoked sundried tomato tapenade, olive oil & balsamic vinegar",
    },
    {
      title: "3B. Selección de Embutidos",
      price: "£15",
      description:
        "Selection of cold meats served with homemade coleslaw, pickled vegetables & toasted bread",
    },
  ],
  meat: [
    {
      title: "4.Empanada de Carne",
      price: "£5.5",
      description:
        "Chilli beef wrapped in puff pastry topped with spicy tomato sauce & melted cheddar",
    },
    {
      title: "5. Alitas de Pollo (GF)",
      price: "£5",
      description: "Marinated chicken wings glazed in honey <br/>OR <br/>Marinated chicken wings in piri piri sauce",
    },
    {
      title: "6.Chorizo al Vino (GF)",
      price: "£5",
      description: "Pan fried spicy chorizo sausage finished with red wine & marjoram",
    },
    {
      title: "7.Estofado de Cordero (GF)",
      price: "£5.5",
      description: "Moroccan lamb tagine",
    },
    {
      title: "8. Queso de Cabra con Jamón Serrano e Higos (GF)",
      price: "£5",
      description:
        "Goat’s cheese wrapped in serrano ham served with rocket, balsamic vinegar & fig relish",
    },
    {
      title: "9. Pato Confitado (GF)",
      price: "£9",
      description: "Confit duck leg served on a bed of stewed chickpea, spinach & celery",
    },
    {
      title: "10. Carne de Ternera con Salsa de Pimienta",
      price: "£9",
      description:
        "Char-grilled sirloin of beef with pepper sauce served on a bed of basmati rice",
    },
    {
      title: "11. Albóndigas Picantes",
      price: "£5",
      description: "Spicy pork meatballs with a tomato sauce & flaked almond",
    },
    {
      title: "12. Brocheta de Pollo (GF)",
      price: "£5",
      description: "Grilled Cajun spiced chicken skewer on a bed of rocket with aioli",
    },
    {
      title: "13. Morcilla Frita",
      price: "£5",
      description: "Pan fried black pudding, shallots with apple puree",
    },
  ],
  fish: [
    {
      title: "14A. Gambas Rebozadas ",
      price: "£6",
      description:
        "Scampi in a light crispy batter served with homemade tartar sauce",
    },
    {
      title: "14B. Croquetas de Bacalao",
      price: "£5",
      description:
        "Salted Cod croquettes served with tarragon mayonnaise & mix salad",
    },
    {
      title: "15. Vieiras (GF)",
      price: "£9",
      description: "Pan Seared Scallops served on a bed of celeriac & nutmeg puree",
    },
    {
      title: "16. Mejillones a la Marinera (GF)",
      price: "£ See Server",
      description: "Mussels of the day – See Server",
    },
    {
      title: "17. Gambas al Ajillo (GF)",
      price: "£7",
      description: "Tiger prawns served in garlic & chilli oil",
    },
    {
      title: "18. Calamares",
      price: "£6",
      description: " Salted squid served with cayenne, smoked paprika & lime mayonnaise",
    },
    {
      title: "19. Pescado del día frito (GF)",
      price: "£ See Server",
      description:
        "Pan fried catch of the day served with salsa verde & mixed salad",
    },
    {
      title: "20. Pulpo y Chorizo",
      price: "£8",
      description: "Pan fried octopus & chorizo served potato, red onion & mix peppers",
    },
  ],
  vegetarian: [
    {
      title: "21. Buñuelos de queso de cabra",
      price: "£4.5",
      description:
        "Goats cheese fritters served with a rocket, beetroot & balsamic vinegar",
    },
    {
      title: "22. Patatas Bravas (GF) (V)",
      price: "£4",
      description:
        "Crispy fried pototas topped with spicy tomato sauce & aioli",
    },
    {
      title: "23. Tortilla Española (GF)",
      price: "£4",
      description: "Spanish omelette with red pepper, onion & potato",
    },
    {
      title: "24. Pimientos del Padrón",
      price: "£4",
      description:
        "Padron peppers served with cayenne, smoked paprika & lime mayonnaise",
    },
    {
      title: "25. Empanada Vegetariana",
      price: "£4",
      description:
        "Sweet potato, goats cheese and caramelised red onion wrapped in filo pastry with tahini yoghurt",
    },
    {
      title: "26. Champiñones con Pesto y Brie Fundido",
      price: "£4.5",
      description:
        "Large flat cup mushrooms served with fresh pesto & melted brie",
    },
    {
      title: "27. Patatas fritas con salsa de ajo (GF)",
      price: "£4",
      description:
        "Cajun spiced chips served with sweet chilli & sour cream dip",
    },
    {
      title: "28. Croquetas de Patata (GF) ",
      price: "£4",
      description: "Potato croquettes served with aioli & mix salad",
    },
    {
      title: "29. Brioche con Halloumi y mermelada de chile",
      price: "£4.5",
      description: "Grilled halloumi, chilli jam on brioche toast",
    },
  ],
  salads: [
    {
      title: "30. Ensalada Vasca (GF)",
      price: "£5.5",
      description:
        "Salad with goats cheese, avocado, olives, tomato, red onion & balsamic vinegar",
    },
    {
      title: "31. Ensalada Mixta (GF)(V)",
      price: "£4",
      description: "Mixed leaf salad with red onion, avocado & sundried tomato",
    },
    // {
    //   title: "32. Classic Russian Salad ",
    //   price: "£5",
    //   description:
    //     "",
    // },
    {
      title: "33. Jamón serrano con melón (GF)",
      price: "£5",
      description:
        "Serrano ham salad with rocket, galia melon, faked almond and balsamic vinegar",
    },
  ],
  paella: [
    {
      title: "35. Paella de Marisco (GF)",
      price: "£20",
      description:
        "Seafood paella with squid, tiger prawns, mussels, langoustine & fish of the day in saffron and spicy tomato rice",
    },
    {
      title: "36. Paella Valenciana (GF)",
      price: "£20",
      description:
        "Mixed paella with chicken, chorizo, mussels, langoustine and tiger prawns in saffron and spicy tomato rice",
    },
    {
      title: "37. Paella de Carne (GF)",
      price: "£20",
      description:
        "Meat paella with chicken and chorizo in saffron and spicy tomato rice",
    },
    {
      title: "38. Paella Vegetal (GF)",
      price: "£14",
      description:
        "Vegetarian paella with mushrooms, peppers, asparagus mixed vegetables in saffron and spicy tomato rice",
    },
  ],
  desserts: [
    {
      title: "Brownie de chocolate con helado",
      price: "£6",
      description:
        "Warm chocolate & Guinness brownie served with vanilla ice cream ",
    },
    {
      title: "Crema Catalana (GF)",
      price: "£6",
      description: "Orange infused crème brûlée",
    },
    {
      title: "Tarta de Queso",
      price: "£6",
      description: "Freshly baked cheese cake of the day",
    },
    {
      title: "Selección de helados (GF)",
      price: "£6",
      description:
        "Selection of Chocolate, honeycomb & vanilla ice cream served with Bailey’s liquor",
    },
    {
      title: "Quesos",
      price: "£8",
      description:
        "Selection of house cheeses with crackers, fig relish and fresh fruit",
    },
  ],
  // lunch: [
  //   {
  //     title: "Pasta Arrabiatta",
  //     price: "",
  //     description:
  //       "Spicy tomato and pepper pasta with a choice of chicken, chorizo, tiger prawns or mixed vegetables",
  //   },
  //   {
  //     title: "Beef Burger",
  //     price: "",
  //     description:
  //       "Served with mixed salad, onion rings and chips Choice of pepper sauce, tomato ketchup and aioli (Add brie or cheddar £1.00)",
  //   },
  //   {
  //     title: "Chicken Burger",
  //     price: "",
  //     description:
  //       "Dressed with pesto and brie mixed salad, onion rings and chips Choice of pepper sauce, tomato ketchup and aioli",
  //   },
  //   {
  //     title: "Steak Sandwich",
  //     price: "",
  //     description:
  //       "Steak Sandwich on a bed of salad served with chips, onion rings and pepper sauce (Add brie or cheddar £1.00)",
  //   },
  //   {
  //     title: "Scampi & Chips",
  //     price: "",
  //     description:
  //       "Scampi in a light and crispy batter, served with chips, mixed salad and homemade tartar sauce",
  //   },
  // ],
  // mains: [
  //   {
  //     title: "10  oz Sirloin steak",
  //     price: "£24",
  //     description:
  //       "10oz Sirloin steak served with pepper sauce, mushrooms, tomato, side salad and choice of potatoes",
  //   },
  //   {
  //     title: "Scampi & Chips",
  //     price: "£13",
  //     description:
  //       "Scampi in a light and crispy batter, served with mixed salad, chips and homemade tartar sauce",
  //   },
  //   {
  //     title: "Steak sandwich",
  //     price: "£11 (add brie or cheddar - £1)",
  //     description:
  //       "Steak sandwich on bed of salad served with chips onion rings and pepper sauce ",
  //   },
  //   {
  //     title: "2Taps Christmas dinner",
  //     price: "£13",
  //     description:
  //       "Turkey, ham & stuffing served with roasted baby potatoes, carrot & parsnip in honey, gravy & cranberry sauce",
  //   },
  // ],
  // kids: [
  //   {
  //     title: "Beef burger served with chips",
  //     price: "",
  //     description: "Hamburguesa de ternera servida con patatas fritas",
  //   },
  //   {
  //     title: "Chef’s battered scampi served with chips and peas",
  //     price: "",
  //     description: "Goujons depollo del chef",
  //   },
  //   {
  //     title: "Chef’s chicken goujons served with chips",
  //     price: "",
  //     description: "Pollo del chef servido con patatas fritas",
  //   },
  //   {
  //     title: "Catch of The Day",
  //     price: "",
  //     description: "Catch of the day served with chips & peas",
  //   },
  //   {
  //     title: "Ice Cream for dessert",
  //     price: "",
  //     description: "El helado de postre",
  //   },
  // ],
  vegan: [
    {
      title: "Aceitunas – Mixed Olives",
      price: "$4",
      description: "Ask server for varieties",
    },
    {
      title: "Pan Fresco – Bread Basket",
      price: "£6",
      description: "Olive oil & balsamic vinegar, Sundried tomato tapenade, Roast pepper hummus",
    },
    {
      title: "PANES PLANOS",
      price: "£6",
      description: "Grilled flat bread served with roast pepper hummus, black olives tapenade, sundried tomato tapenade and Spanish guacamole",
    },
    {
      title: "Patatas Bravas",
      price: "£4",
      description: "Crispy fried potatoes with spicy tomato sauce and garlic aioli",
    },
    {
      title: "PATATAS ASADAS CON ROMERO",
      price: "£4",
      description: "Roasted new potatoes with garlic, rosemary and sea salt",
    },
    {
      title: "Pan Frito Crujientes Verdes",
      price: "£4",
      description: "Pan-fried crunchy greens with roast pepper hummus",
    },
    {
      title: "Champignons",
      price: "£4.50",
      description: "Flat cap mushrooms, topped with tofu",
    },
    {
      title: "Patatas Fritas con salsa",
      price: "£4",
      description: "Cajun spiced chips with sweet chilli dipping sauce",
    },
    {
      title: "Halloumi y Mermelada",
      price: "£5",
      description: "Grilled Tofu, and chilli jam on toast",
    },
    {
      title: "Mixta",
      price: "£4",
      description: "Mixed leaves with red onion, avocado, and sundried tomato",
    },
    {
      title: "Paella Vegetal",
      price: "£14",
      description: "Vegetarian paella with mushrooms, peppers, asparagus, mixed vegetables in saffron and spicy tomato rice",
    },
   
  ],
}
